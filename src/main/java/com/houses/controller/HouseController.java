...
import com.houses.common.model.VerifyCode;
import com.houses.common.vo.HouseMainInfoVo;
import com.houses.dao.IVerifyCodeGen;
import com.houses.service.ICreatePDFService;
import com.houses.service.IHouseService;
import com.houses.service.impl.SimpleCharVerifyCodeGenImpl;
...
//			return "home.html";
//		} else {
//			System.out.println(request.getSession().getAttribute("username").toString() + "用户无超级管理员权限");
//			return "userHome.html";
//		}
//		
//	}
	...
	@ResponseBody
	public int loginPost(@RequestBody User user, HttpServletRequest request) {
		
//		System.out.println(request.getSession().getAttribute("verifyCode").toString().equals(user.getVerifyCode()));
//		System.out.println(request.getSession().getAttribute("verifyCode").toString());
//		String x = request.getSession().getAttribute("VerifyCode").toString();
//		System.out.println(x);
		
		if(!request.getSession().getAttribute("VerifyCode").toString().equals(user.getVerifyCode())) {
			return 10;
		}
		...
		if (result.equals("登陆成功")) {
			...
			request.getSession().setAttribute("username", user.getUser());
				return 0;
			} else {
			return 2;
			//"/login.html";
...
//	@ApiOperation(value = "验证码")
	...
	public void verifyCode(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			// 设置长宽
			VerifyCode verifyCode = iVerifyCodeGen.generate(80, 35);
...
//         fileIs = new FileInputStream(imgFile);
			Thumbnails.of(imgFile).size(100, 100).outputFormat("png").toOutputStream(outStream);
		} catch (Exception e) {
...
//        fileIs.close();   
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response, Integer houseId) throws IOException {
		...
		// 否则直接使用response.setHeader("content-disposition", "attachment;filename=" +
		// filename);即可
